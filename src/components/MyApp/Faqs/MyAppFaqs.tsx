...
import { MyAppFaqData } from "@/data/myApps/myAppFaqsData";
import {
  Box,
  Collapsible,
  GridItem,
...
const MyAppFaqs: React.FC<Props> = (props: Props) => {
        ...
        <GridItem>
          <VStack align={"flex-start"} gap={4}>
            <Tag.Root
              ...
              size={{ base: "lg", md: "xl" }}
            >
              <Tag.Label>{faqsData.header.badge}</Tag.Label>
            </Tag.Root>
            <Heading
              as="h3"
              fontWeight={"bold"}
              ...
              lineHeight={"normal"}
            >
              {faqsData.header.title}
            </Heading>
            <Text color={"fg.muted"}>{faqsData.header.desc}</Text>
          </VStack>
        </GridItem>
        <GridItem>
