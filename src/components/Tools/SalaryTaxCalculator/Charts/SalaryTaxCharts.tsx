  ...
  return (
    ...
    >
      ...
      {payload.map((entry, i) => (
        <div
          key={i}
          style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: i < payload.length - 1 ? "4px" : 0 }}
        >
          <span
            style={{
...
const SalaryTaxCharts: React.FC<SalaryTaxChartsProps> = ({ result }) => {
  useColorPalette(); // keep context subscription, colors use fixed vibrant values
  ...
  // Vibrant fixed colors — high contrast in both light and dark mode
  const TAKE_HOME_COLOR = "#10b981"; // emerald
  const TAX_COLOR = "#f97316";       // orange
